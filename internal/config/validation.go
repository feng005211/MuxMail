...
const maxMVPAttemptsPerMessage = 3
...
func (v *configValidator) validateProviderAccounts() {
	...
	for index, account := range v.cfg.ProviderAccounts {
		...
		if account.Provider == domain.ProviderResend || account.Provider == domain.ProviderBrevo {
			if strings.TrimSpace(credentials["api_key"]) == "" {
				v.addError("secret_ref_required", path+".credentials.api_key", "provider credentials.api_key is required")
			}
		}
		for name, ref := range credentials {
			credentialPath := path + ".credentials." + name
			v.validateSecretRef(credentialPath, ref)
		}
	}
}
...
func (v *configValidator) validateWebhooks() {
	...
	v.validateSecretRef("webhooks.shared_secret_ref", v.cfg.Webhooks.SharedSecretRef)
	if strings.TrimSpace(v.cfg.Webhooks.ResendSecretRef) != "" {
		v.validateSecretRef("webhooks.resend_secret_ref", v.cfg.Webhooks.ResendSecretRef)
	}
	if strings.TrimSpace(v.cfg.Webhooks.BrevoTokenRef) != "" {
		v.validateSecretRef("webhooks.brevo_token_ref", v.cfg.Webhooks.BrevoTokenRef)
	}
}
...
func (v *configValidator) validateAPIKeys(appPath string, app AppConfig) {
	...
	for keyIndex, key := range app.APIKeys {
		...
		v.validateSecretRef(path+".key_ref", key.KeyRef)
	}
}
...
func (v *configValidator) validateFromDomain(path string, from string, senderDomain string) {
	...
	fromDomain, ok := emailDomain(from)
	if !ok {
...
func (v *configValidator) validateSecretRef(path string, ref string) {
